...
describe("listProfiles", () => {
  ...
  it("rejects invalid profile names before invoking the Hermes CLI", async () => {
    expect((await createProfile("../outside", true)).success).toBe(false);
    expect((await createProfile("-flag", true)).success).toBe(false);
    expect((await deleteProfile("../outside")).success).toBe(false);
    await expect(setActiveProfile("../outside")).rejects.toThrow(
