...
export const ChatInput = forwardRef<ChatInputHandle, ChatInputProps>(
  function ChatInput(
    ...
    const filteredSlashCommands = useMemo(
      () =>
        slashMenuOpen ? filterSlashCommands(SLASH_COMMANDS, slashFilter) : [],
    ...
    function handleKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>): void {
      ...
      if (slashMenuOpen && filteredSlashCommands.length > 0) {
        ...
        if (e.key === "Enter" || e.key === "Tab") {
          // If the filter has zero matches, treat Enter as "submit raw
          // text" (let the parent route the slash string through normal
          // send) and just close the menu — never dereference an out-of-
          // bounds index.
          ...
          // Fall through to default Enter behavior (send).
          setSlashMenuOpen(false);
        }
        if (e.key === "Escape") {
          e.preventDefault();
