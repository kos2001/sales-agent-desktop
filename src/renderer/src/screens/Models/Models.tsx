...
// Shared cache key for the user's saved models. Other screens that mutate the
// model list (Providers, Chat's model picker) should invalidate this key so
// the Models screen refreshes without prop-drilling a `visible` reload hack.
...
function Models({ visible }: ModelsProps = {}): React.JSX.Element {
  ...
  const {
    ...
    queryFn: () => window.hermesAPI.listModels(),
  });
  ...
  // Refetch when the pane becomes visible. Other screens (Providers,
  // chat model picker) still mutate models without invalidating the
  // cache, so this keeps existing behavior until those write sites are
  // migrated to invalidate `modelsQueryKey` directly.
  useEffect(() => {
    if (visible) refetch();
  }, [visible, refetch]);
