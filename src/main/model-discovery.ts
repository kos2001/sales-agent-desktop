...
import { getCustomRequestHeaders, readEnv } from "./config";
import {
 ...
 *  Exported so session-summary.ts can reach the configured model. */
export function canonicalBaseUrl(provider: string): string | null {
...
export async function discoverProviderModels(
  ...
  // Merge in any custom request headers from the active profile's .env
  // (SERVICE_ID, USER_ID, OPENAI_HEADER_*). A corporate proxy fronting
  // OpenRouter typically requires these on the /v1/models endpoint
  // too — same gateway, same audit requirements.
  const headers = {
