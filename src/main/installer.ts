...
// A populated hermes home contains at least one of five marker
// children. Earlier this fired six syscalls (existsSync(dir) + five
// existsSync(join(dir, name))); now one readdirSync yields the same
// signal in a single syscall. Set lookups are O(1), the directory has
// at most a couple-dozen entries, and the bail-out on ENOENT covers
// the "dir doesn't exist" case implicitly.
...
function looksLikeHermesHome(dir: string): boolean {
  ...
  return HERMES_HOME_MARKERS.some((m) => set.has(m));
}
...
export function getEnhancedPath(): string {
  ...
  // Bundled tools (uv) go first so the upstream install script detects them as
  // already-installed and skips the hardcoded astral.sh download — the one
  // network hop that proxy/mirror env vars can't redirect. See
  // resolveBundledToolBins.
  const bundledBins = resolveBundledToolBins();
  return [...bundledBins, ...extra, process.env.PATH || ""]
    .filter(Boolean)
    .join(delimiter);
}
...
// Bundling the script is what makes installs work when raw.githubusercontent
// is blocked — see scripts/vendor-install-scripts.mjs.
// Candidate absolute paths for a bundled resource <relPath> across prod
// (process.resourcesPath) and dev (repo resources/) layouts.
function bundledResourceCandidates(relPath: string): string[] {
  const resourcesPath =
    (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath ??
    "";
  return [
    resourcesPath ? join(resourcesPath, "resources", relPath) : "",
    resourcesPath ? join(resourcesPath, relPath) : "",
    join(__dirname, "..", "..", "resources", relPath),
    join(process.cwd(), "resources", relPath),
  ].filter(Boolean);
}

export function resolveBundledScript(name: string): string | null {
  for (const c of bundledResourceCandidates(name)) {
    try {
...
// Locate bin directories for tools bundled under resources/ so they can be put
// on PATH for the install subprocess. Currently just uv: when the upstream
// install script finds uv already on PATH it skips the hardcoded astral.sh
// download — the single network hop that proxy/mirror env vars cannot
// redirect. Returns the dir(s) containing a bundled uv binary, or [] when
// nothing is bundled. See scripts/vendor-install-scripts.mjs.
export function resolveBundledToolBins(): string[] {
  const uvName = IS_WINDOWS ? "uv.exe" : "uv";
  const relPaths = [join("uv", uvName), uvName];
  const bins = new Set<string>();
  for (const rel of relPaths) {
    for (const c of bundledResourceCandidates(rel)) {
      try {
        if (existsSync(c)) bins.add(dirname(c));
      } catch {
        /* ignore */
      }
    }
...
/**
 * Restart the local Hermes gateway daemon so it re-reads `config.yaml`
 * and `.env`. Used after `ensureApiServerTokenSync()` rewrites the
 * gateway secret — the running Python backend caches the token in
 * memory at startup, so without a restart it keeps rejecting requests
 * that carry the freshly-synced token. No-op in remote/SSH mode; the
 * desktop doesn't manage gateways it doesn't own.
 */
export async function restartHermesGateway(): Promise<boolean> {
  if (!existsSync(HERMES_PYTHON) || !existsSync(HERMES_SCRIPT)) return false;
  return new Promise<boolean>((resolve) => {
    execFile(
      HERMES_PYTHON,
      hermesCliArgs(["gateway", "restart"]),
      {
        cwd: HERMES_REPO,
        env: {
          ...process.env,
...
export async function runInstall(
  ...
  // Corporate-network routing (proxy / internal PyPI / git mirror). Computed
  // here so the temp gitconfig path is visible to the finally cleanup.
  const corpCfg = getCorporateNetworkConfig();
  const corpEnv = buildCorporateEnv(corpCfg);
  let tempGitConfig: string | null = null;

  try {
    tempGitConfig = writeTempGitConfig(corpCfg);
    return await new Promise<void>((resolve, reject) => {
      ...
      // don't inherit the terminal environment. Prefer a bundled install.sh so
      // installs work when raw.githubusercontent.com is blocked.
...
async function runInstallWindows(emit: (t: string) => void): Promise<void> {
  ...
  // Corporate-network routing (proxy / internal PyPI / git mirror).
  const corpCfg = getCorporateNetworkConfig();
  const corpEnv = buildCorporateEnv(corpCfg);
  ...
  // Prefer a bundled install.ps1 so installs work when raw.githubusercontent
  // is blocked. Otherwise download it to a temp file (the wrapper sidesteps the
  // `iex`-can't-pass-args limitation) and invoke with our parameters.
  const bundledPs1 = resolveBundledScript("install.ps1");
  const invokeLines = bundledPs1
    ? [
    ...
    : [
        ...
        // Windows PowerShell 5.1 parses BOM-less files as the legacy ANSI
        // codepage, which mangles the non-ASCII glyphs in install.ps1 and
        // produces parse errors (see issue #149). Re-save with a UTF-8 BOM so
        // PS 5.1 reads it as UTF-8. Idempotent if upstream adds its own BOM.
