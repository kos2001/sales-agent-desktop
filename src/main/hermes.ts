...
function sendMessageViaApi(
  ...
  // Custom request headers for OpenAI-compatible gateways that demand
  // extra identity fields per request (e.g. `Service-Id`, `User-Id`,
  // or any `OPENAI_HEADER_<NAME>` defined in the profile's .env). See
  // config.ts:getCustomRequestHeaders for the precedence rules.
  // Spread last so the user can NOT accidentally override
  // Content-Type / Authorization via OPENAI_HEADER_AUTHORIZATION — we
  // re-assert those above this line. If the user really needs that,
  // they should remove API_SERVER_KEY instead of fighting the merge.
  const customHeaders = getCustomRequestHeaders(profile);
  for (const [name, value] of Object.entries(customHeaders)) {
    // Skip names that would collide with headers we've already
    // explicitly set — Content-Type, Authorization, X-Hermes-Session-Id.
    const lower = name.toLowerCase();
    if (
      lower === "content-type" ||
      lower === "authorization" ||
      lower === "x-hermes-session-id"
    ) {
      continue;
    }
    headers[name] = value;
  ...
  const req = requester(
    ...
    (res) => {
      ...
      res.on("data", (chunk: Buffer) => {
        // indexOf-based slicing instead of split — most chunks contain
        // zero or one complete SSE block, so a full split (which
        // allocates an array of every part on every chunk) is wasted
        // work. For a 10KB response that arrives in 50 packets we'd
        // otherwise allocate 50 arrays just to throw 49 of them away.
      ...
      res.on("end", () => {
        // Drain any partial trailing block. Same loop shape as the
        // on('data') hot path so behaviour matches.
        if (buffer.trim()) {
          let nl: number;
          while ((nl = buffer.indexOf("\n\n")) !== -1) {
            const block = buffer.slice(0, nl);
          ...
          // The trailing fragment (no double newline) is a complete
          // block at end-of-stream — feed it through.
          if (buffer.trim()) {
...
export async function sendMessage(
  ...
  // NOTE: per-conversation PII protection (de-identify outgoing text +
  // re-identify the response) is applied in the `send-message` IPC handler
  // (index.ts) when the user enabled protection for the conversation — it
  // needs to coordinate with response streaming for re-identification. By
  // the time text reaches here it is already de-identified when protected.
