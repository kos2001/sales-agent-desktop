...
// ── API server token sync ────────────────────────────────
//
// The gateway-secret historically lives in several places:
//   - `.env` `API_SERVER_KEY`              (read by desktop client)
//   - `config.yaml` top-level `api_server.token`  (read by desktop client)
//   - `config.yaml` `platforms.api_server.token`  (newer Python backend
//                                                  nests it under platforms)
//
// `hermes setup` writes to one of these; the user (or this app) may
// touch another. When they drift, chat fails on the second message with
// "Session continuation requires API key authentication" (#333 and the
// long thread that followed). `ensureApiServerTokenSync` is the
// boot-time reconciler: pick a single canonical value (first non-empty
// in priority order, or generate one if all empty) and propagate it to
// every location that already exists. Locations that don't exist are
// left alone so we don't impose schema on installs that intentionally
// don't use `api_server`.
//
// Returns the chosen token plus a `changed` flag so the caller can
// decide whether to restart the gateway (the running Python backend
// caches its token in memory at startup).
export type ApiServerSyncReason =
  | "in-sync"
  | "synced-from-existing"
  | "generated";

export interface ApiServerSyncResult {
  token: string;
  changed: boolean;
  reason: ApiServerSyncReason;
}

export function ensureApiServerTokenSync(
  profile?: string,
): ApiServerSyncResult {
  const envValue = (readEnv(profile).API_SERVER_KEY ?? "").trim();
  const topLevelToken = (
  ...
  const platformsToken = (
    getConfigValue("platforms.api_server.token", profile) ?? ""
  ).trim();

  // Priority for picking the canonical value when locations disagree.
  // `.env` first because it's the documented user override; then the
  // canonical hermes-agent top-level path; legacy/platforms last.
  const canonical =
    [envValue, topLevelToken, topLevelLegacy, platformsToken].find((v) => v) ||
    "";

  let token = canonical;
  let reason: ApiServerSyncReason = "in-sync";
  ...
  let changed = reason === "generated";

  // Always ensure `.env` has the canonical value — the desktop client
  // reads this for chat requests.
  if (envValue !== token) {
    setEnvValue("API_SERVER_KEY", token, profile);
    changed = true;
    if (reason === "in-sync") reason = "synced-from-existing";
  }

  // Only write into config.yaml locations that already exist. setConfigValue
  // bails on missing nested paths, so this is safe by construction, but the
  // explicit check lets us skip the no-op call entirely.
  if (topLevelToken && topLevelToken !== token) {
    setConfigValue("api_server.token", token, profile);
    changed = true;
    if (reason === "in-sync") reason = "synced-from-existing";
  }
  if (topLevelLegacy && topLevelLegacy !== token) {
    setConfigValue("API_SERVER_KEY", token, profile);
    changed = true;
    if (reason === "in-sync") reason = "synced-from-existing";
  }
  if (platformsToken && platformsToken !== token) {
    setConfigValue("platforms.api_server.token", token, profile);
    changed = true;
    if (reason === "in-sync") reason = "synced-from-existing";
  }

  return { token, changed, reason };
}

// ── Custom request headers (OpenAI-compatible providers) ─────
//
// Some OpenAI-compatible gateways require additional headers per
// request — e.g. an internal corporate router that demands
// `Service-Id` and `User-Id` for billing or audit. These have no place
// in `Authorization: Bearer …` and the existing /v1/chat/completions
// path doesn't accept them as body fields.
//
// We support two configurable kinds, both per-profile, both read from
// `.env`:
//
//   1. `SERVICE_ID=…`  → header `Service-Id: …`
//   2. `USER_ID=…`     → header `User-Id: …`
//   3. `OPENAI_HEADER_<NAME>=…` (extensibility hook). For example
//      `OPENAI_HEADER_X_TENANT=acme` becomes `X-Tenant: acme`.
//      Underscores in the suffix become hyphens; the suffix is
//      title-cased on the request to match common HTTP header style.
//
// Profile env wins over default env (same precedence as API_SERVER_KEY).
// Result is cached for the same 5s TTL as readEnv() / getApiServerKey().
export function getCustomRequestHeaders(
  profile?: string,
): Record<string, string> {
  const cacheKey = `customHeaders:${profile || "default"}`;
  const cached = getCached<Record<string, string>>(cacheKey);
  if (cached !== undefined) return cached;
  ...
  const headers = resolveCustomRequestHeaders(env, fallback);
  setCache(cacheKey, headers);
  return headers;
}

/**
 * Pure resolver — extracted so the precedence policy is unit-testable
 * without filesystem fixtures. Profile values shadow default values.
 */
export function resolveCustomRequestHeaders(
  profileEnv: Record<string, string>,
  defaultEnv: Record<string, string>,
): Record<string, string> {
  // Walk default env first, then profile env on top so profile wins.
  ...
  for (const src of [defaultEnv, profileEnv]) {
    for (const [key, raw] of Object.entries(src)) {
      ...
      // OPENAI_HEADER_<SUFFIX> → <Suffix> (underscores → hyphens, title-case)
...
/**
 * `X_TENANT_GROUP` → `X-Tenant-Group`. Splits on underscore, title-
 * cases each part. Reused by tests; exported.
 */
export function suffixToHeader(suffix: string): string {
  return suffix
    .split("_")
    .filter(Boolean)
