...
// Set inside the auto-updater setup; defaults to "no update" in dev/portable
// builds where electron-updater is not wired up.
let checkForUpdatesImpl: () => Promise<string | null> = async () => null;
let _desktopUpdateCache: { at: number; value: string | null } | null = null;
const DESKTOP_UPDATE_TTL_MS = 60_000;
...
function setupIPC(): void {
  ...
  ipcMain.handle("check-version-status", async (): Promise<VersionStatus> => {
    ...
    let engine: VersionStatus["engine"] = {
      current: null,
      latest: null,
    ...
    // Compare against the latest GitHub release of the desktop release repo
    // (fetchLatestDesktopRelease has its own 3s timeout + 1h cache + try/catch
    // → null). electron-updater (checkForUpdatesImpl) is reserved for the actual
    // download/install action, not this comparison.
    const latestDesktop = await fetchLatestDesktopRelease();
    const desktop: VersionStatus["desktop"] = {
  ...
  ipcMain.handle("set-active-profile", (_event, name: string) => {
    // Fire-and-forget — preserves the prior sync handler's "return true
    // immediately" contract. setActiveProfile already swallows its own
    // errors so an unhandled rejection here would only signal a real
    // bug worth surfacing.
    if (getConnectionConfig().mode !== "ssh") {
      void setActiveProfile(name);
    }
    return true;
  });
...
function setupUpdater(): void {
  ...
  checkForUpdatesImpl = async () => {
    ...
    try {
      ...
      const value = result?.updateInfo?.version ?? null;
      _desktopUpdateCache = { at: Date.now(), value };
      return value;
    } catch {
      _desktopUpdateCache = { at: Date.now(), value: null };
      return null;
    }
  };

  ipcMain.handle("check-for-updates", async () => checkForUpdatesImpl());
...
app.whenReady().then(() => {
  ...
  // Python gateway always use the same token. Only runs in local mode;
  // for remote/SSH the user manages the gateway themselves. If the
  // token actually changed (drift detected, or generated fresh), the
  // running gateway has a stale in-memory copy — restart it.
  (() => {
    const conn = getConnectionConfig();
    if (conn.mode !== "local") return;
    try {
      const result = ensureApiServerTokenSync();
      if (result.changed) {
        console.log(
          `[api-server-key] ${result.reason} — restarting gateway to pick up new token`,
        );
        restartHermesGateway().catch((err) =>
          console.error("[api-server-key] gateway restart failed:", err),
        );
      }
    } catch (err) {
      console.error("[api-server-key] sync failed:", err);
    }
  })();
...
app.on("before-quit", () => {
  ...
  stopSshTunnel();
  stopClaw3d();
  // Close pooled SQLite handles so WAL gets checkpointed.
  closeSessionsDbs();
});
