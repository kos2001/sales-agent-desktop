...
- **Custom request headers** — stamp per-profile `Service-Id`, `User-Id`, or arbitrary `OPENAI_HEADER_*` headers onto every chat and model-discovery request for corporate gateways fronting OpenAI-compatible APIs ([docs](docs/CUSTOM-REQUEST-HEADERS.md))
...
Some deployments sit OpenAI-compatible providers (OpenRouter, OpenAI, etc.) behind a corporate gateway that requires extra identity headers — typically `Service-Id` and `User-Id` for billing or audit. Hermes Desktop reads these from the active profile's `.env` and merges them into every outgoing chat and model-discovery request.

| `.env` key             | Resulting header   |
| ---------------------- | ------------------ |
| `SERVICE_ID`           | `Service-Id`       |
| `USER_ID`              | `User-Id`          |
| `OPENAI_HEADER_<NAME>` | `<Name>` (title-cased; underscores → hyphens) |

`Service-Id` and `User-Id` are also editable from **Settings → Custom Request Headers**. `Authorization`, `Content-Type`, and `X-Hermes-Session-Id` are protected — they can't be overridden through this mechanism.

To check which headers will be sent without firing a real request:

```bash
node scripts/check-custom-headers.mjs
```

Full reference: [`docs/CUSTOM-REQUEST-HEADERS.md`](docs/CUSTOM-REQUEST-HEADERS.md).
